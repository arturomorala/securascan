...
def main():
    ...
    try:
        ...
        user = User(email=f"e2e-{suffix}@example.test", password_hash="E2E_DISABLED", first_name="E2E", status="DISABLED")
        db.add(user)
        db.flush()
        org = Organization(name=f"E2E {suffix}", slug=f"e2e-{suffix}", owner_user_id=user.id, plan_code="FREE")
        db.add(org)
        db.flush()
